...
(defn -path [[x y] move]
  (let [dir      (subs move 0 1)
        cnt      (Integer/parseInt (subs move 1))
        r        (range 1 (inc cnt))]
    (for [point r]
      (case dir
        "U" [x (+ y point)]
...
(defn path [moves]
  (reduce (fn [acc move]
            (concat acc (-path (or (last acc) [0 0]) move)))
          (list)
          (s/split moves #",")))


(defn d+s [intersections]
  (reduce (fn [acc {:keys [ix steps]}]
            (conj acc {:dist  (+ (Math/abs (first ix))
                                 (Math/abs (last  ix)))
                       :steps (+ (inc (first steps)) ;; because index is 0 based
                                 (inc (last  steps)))}))
          []
          intersections))

(defn intersections [p1 p2]
  (let [ixs (clojure.set/intersection (set p1) (set p2))]
    (reduce (fn [acc ix]
              (conj acc
                    (let [steps [(.indexOf p1 ix) (.indexOf p2 ix)]]
                      {:ix    ix
                       :steps steps})))
            []
            ixs)))

(defn day3 [wire1 wire2]
  (let [path1     (-> wire1 path)
        path2     (-> wire2 path)
        ixs       (intersections path1 path2)
        distances (d+s ixs)]
    {:lowest-distance (->> distances (mapv :dist)  sort first)
     :lowest-steps    (->> distances (mapv :steps) sort first)}))
...
(comment
  ...
  (distance
   #{[155 11] [146 46] [155 4] [158 -12]})

  (is (= 6   (:lowest-distance (day3 "R8,U5,L5,D3" "U7,R6,D4,L4"))))
  (is (= 159 (:lowest-distance (day3 "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83"))))
  (is (= 410 (:lowest-steps (day3 "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83"))))
  (is (= 135 (:lowest-distance (day3 "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51"
